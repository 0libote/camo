  ...
  return (
    <div className="min-h-screen bg-bo7-black text-slate-200 p-4 md:p-8 relative selection:bg-bo7-orange/30 selection:text-bo7-orange">
      ...
      <div
        ...
        style={{
          background: `radial-gradient(600px circle at ${mousePos.x}px ${mousePos.y}px, rgba(255, 159, 0, 0.03), transparent 40%)`
        }}
      />
      ...
      <div className="relative z-10">
        ...
        <main className="max-w-7xl mx-auto space-y-12">
          ...
          <div className="space-y-16">
            {viewMode === 'classes' ? (
              WEAPON_CLASSES.map(className => (
                <WeaponList
                  key={className}
                  className={className}
                  weapons={CAMO_DATA.weapons.filter((w: Weapon) => w.class === className)}
                  progress={progress}
                  onToggle={toggleCamo}
                />
              ))
